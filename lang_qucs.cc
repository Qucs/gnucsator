			...
			if (!cmd.more()) { untested();
				...
				if (cmd.stuck(&here)) { untested();
					...
					try{ untested();
						...
						}else if (QucsGuessParam(value)){ untested();
...
{ untested();
	...
	if (first_letter == '.') {
		...
		return id_string;
	}else if (!cmd.scan(":")){ untested();
		cmd >> id_string;
	}else{ untested();
		cmd.reset();
		id_string = cmd.get_to(":");
		cmd.reset(here);
		if(id_string=="Sub"){ untested();
