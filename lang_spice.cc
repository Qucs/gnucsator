...
private: // local
  ...
  void print_instance_plain(OMSTREAM&, const COMPONENT*) /*const*/;
  void print_instance_caneda(OMSTREAM&, const COMPONENT*, std::string const&)const;
};
/*--------------------------------------------------------------------------*/
...
public:
  ...
  bool case_insensitive()const override { untested();return true;}
  UNITS units()const override { untested();return uSPICE;}
} lang_acs;
DISPATCHER<LANGUAGE>::INSTALL
...
{ untested();
  ...
  for (;;) { untested();
	...
	num_nodes = minnodes;
      }else if (i <= maxnodes + leave_tail) { untested();
...
{ untested();
  ...
  try{ untested();
	...
	break; // done.  have closing paren.
      }else if (index >= num_nodes) { untested();
...
{ untested();
  ...
  if (gotnodes < x->min_nodes()) { untested();
...
{ untested();
  ...
  if (!x->use_obsolete_callback_parse()) { untested();
    int paren = cmd.skip1b('(');
	...
	if (cmd.stuck(&here)) {untested();
	  ...
	  try{ untested();
	    ...
	    if(Name=="$mfactor" && xx && add_mfactor) { untested();
	      xx->COMPONENT::set_param_by_name("$mfactor", value);
	    }else if(Name == "m" && xx && alias_m_mfactor) { untested();
	      try{ untested();
...
std::string LANG_SPICE_BASE::find_type_in_string(CS& cmd)
{
  ...
  case 'U':
    here = cmd.cursor();
    if (cmd.scan("and |nand |or |nor |xor |xnor |inv ")) { untested();
      s = cmd.trimmed_last_match();
    }else{untested();
      s = "U";
    }
...
/*--------------------------------------------------------------------------*/
void LANG_SPICE_BASE::print_instance_plain(OMSTREAM& o, const COMPONENT* x)
{
  print_label(o, x);
  print_ports(o, x);
  print_type(o, x);
...
/*--------------------------------------------------------------------------*/
static std::string port_value(std::string name, COMPONENT const* x)
{
  assert(x);
  for(int i=0; i<x->net_nodes(); ++i){
    if(x->port_name(i) == name){
      return x->port_value(i);
    }else{
    }
  }
  { untested();
...
/*--------------------------------------------------------------------------*/
static std::string param_value(std::string name, COMPONENT const* x)
{
  assert(x);
  for(int i=0; i<x->param_count(); ++i){
    if(x->param_name(i) == name){
      return x->param_value(i);
    }else{
      trace1("miss", x->param_name(i));
    }
  }
  { untested();
    return "??" + name + "??";
...
/*--------------------------------------------------------------------------*/
void LANG_SPICE_BASE::print_instance_caneda(OMSTREAM& o, const COMPONENT* x,
    std::string const& s) const
{
  ...
  }while (cmd.ns_more() && !cmd.stuck(&here));
...
void LANG_SPICE_BASE::print_instance(OMSTREAM& o, const COMPONENT* x)
{
  if(has_attributes(x->id_tag())) {
    ATTRIB_LIST_p p = attributes(x->id_tag());

    if(p) {
      std::string tpl = p->operator[]("spice_caneda");
      if(tpl.size()){
	return print_instance_caneda(o, x, tpl);
      }else{ untested();
      }
...
public:
  void do_it(CS& cmd, CARD_LIST* Scope)override { untested();
    ...
    if (more_stuff != "") { untested();
      cmd.reset(here);
      getmerge(cmd, NO_HEADER, Scope);
    }else{ untested();
      for (;;) { untested();
	cmd.get_line(section_name + '>');
	if (cmd.umatch(".endl {" + section_name + "}")) { untested();
	  break;
	}else{ untested();
	  // skip it
	}
