...
void Gnucap::copy_param_values(Symbol const* sym, COMPONENT* model)
{
	for(index_t i=0; i<sym->paramCount(); ++i){
		std::string name = sym->paramName(i);
		std::string value = sym->paramValue(i);
...
void Gnucap::load_symbol(Symbol const* i, BASE_SUBCKT* model)
{
	assert(model->scope());
	assert(OPT::language);
	CARD const* proto = OPT::language->find_proto(i->typeName(), model);
	if(proto){
		trace3("load", i->typeName(), i->label(), i->has_common());
		...
		trace3("unknown", i->typeName(), i->label(), i->has_common());
		incomplete();
		return;
	}
}
...
void Gnucap::do_it(istream_t& cs, ElementList const* ctx)
{
	...
	trace0("================= list ============");
	command("list", _main);
	trace2("============== uninit =============", &_sim_data, _sim_data.is_first_expand());
//	_sim_data.uninit();
