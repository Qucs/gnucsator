...
void finish_hack(SIM* s)
{
  ...
  //
  auto& PL = SIM::_probe_lists->store[s_TRAN];
  unsigned number_of_probes = PL.size();

  auto& outFile = std::cout;
  unsigned n=17; //todo
  outFile << "<indep time " << n << ">" << std::endl; //<indep time 19>
  std::vector<WAVE::const_iterator> w;

  for(unsigned i=0; i<number_of_probes; ++i){
    w.push_back(CKT_BASE::_sim->_waves[i].begin());
  }
